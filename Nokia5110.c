...
void static SendByte(uint8_t byte){
    ...
    uint8_t localByte = byte;
    int i;
    for (i=0;i<8;i++){
        if((localByte>>7)>=1){ //If MSB == 1
            LCD_SEND_1; //Assign MOSI Value of 1
        }else{
            LCD_SEND_0; //Assign MOSI Value of 0
        }
        localByte=localByte<<1; //Shift MSB out
        LCD_TOGGLE_SCLK; //Toggle Clock Twice
        LCD_TOGGLE_SCLK;
    }
}
...
void static LCDCommandWrite(uint8_t command){
    // write code here for LCD lab
    LCD_ENABLE_MODULE; //enable the module
    SET_DC_FOR_COMMAND; //set the DC bit for command
    SendByte(command); // send the byte containing the command passed in
    LCD_DISABLE_MODULE; //disable the module
}
// Helper function that sends 8-bit data to the LCD.
void static LCDDataWrite(uint8_t data){
    // write code here for LCD lab
    LCD_ENABLE_MODULE; //enable the module
    SET_DC_FOR_DATA; //set the DC bit for command
    SendByte(data); // send the byte containing the command passed in
    LCD_DISABLE_MODULE; //disable the module
}
